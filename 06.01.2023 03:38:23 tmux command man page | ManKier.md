...
display-message \[-aINpv\] \[
-c target-client\] \[-d delay\] \[-t target-pane\] \[ message\]
:
    > (alias: `display`)
...
display-popup \[-BCE\] \[-b border-lines\] \[
-c target-client\] \[-d start-directory\] \[-e environment\] \[-h height\] \[-s style\] \[
-S border-style\] \[-t target-pane\] \[
-T title\] \[-w width\] \[-x position\] \[-y position\] \[ shell-command\]
:
    > (alias: `popup`)
...
abduco(1), byobu(1), byobu-keybindings(1), byobu-layout(1), byobu-select-backend(1), byobu-tmux(1), logind.conf(5), pty(7), termy-server(1), tq(1), user_caps(5), xpanes(1).
